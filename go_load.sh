...
cd ${WORK_DIR}
# versionの確認
version=`curl -s -L "http://www.ebi.ac.uk/ols/api/ontologies/go" | jq '.config | .version'`
file_url=`curl -s -L "http://www.ebi.ac.uk/ols/api/ontologies/go" | jq '.config | .id' | sed "s/\"//g"` 
